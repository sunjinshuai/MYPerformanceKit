...
#import "MYPopGestureViewController.h"
#import "MYPageViewController.h"
#import "MYMapViewController.h"
#import "MYCollectionViewController.h"
#import "MYScrollViewController.h"

@interface MYPopGestureViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation MYPopGestureViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"MYScrollViewPopGesture";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        UIViewController *vc;
        if (indexPath.row == 0) {
            vc = [[MYScrollViewController alloc] init];
        } else if (indexPath.row == 1) {
            vc = [[MYCollectionViewController alloc] init];
        } else if (indexPath.row == 2) {
            vc = [[MYMapViewController alloc] init];
        } else if (indexPath.row == 3) {
            vc = [[MYPageViewController alloc] init];
        }
